...
public class Rojo extends JPanel {
	...
	public Rojo() {
		...
		TimerTask pape = new TimerTask() {
			public void run() {
				d2.setText("[Proyecto abigai]: *Cae al suelo*");
			}
			};
		...
		TimerTask papi = new TimerTask() {
			public void run() {
				d3.setText("[Tom]: ¿Carl Estas bien?");
			}
			};
		...
		TimerTask papo = new TimerTask() {
			public void run() {
				d4.setText("[Carl]: Que gran tiro amigo!!");
			}
			};
		...
		TimerTask papu = new TimerTask() {
			public void run() {
				d5.setText("[Retro]: Amigos salgan de ahí Tienen una salida de emergencia al frente.");
			}
			};
		...
		TimerTask papaa = new TimerTask() {
			public void run() {
				d6.setText("[Tom]: Ok Retro Vamos para allá");
			}
			};
			pepa.schedule(papaa,6000);
		TimerTask papee = new TimerTask() {
			public void run() {
				d7.setText("[Tom y Carl]: *Se desconetan*");
			}
			};
